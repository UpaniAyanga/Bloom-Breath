  ...
  return (
    <div className="relative flex h-64 w-64 items-center justify-center rounded-full bg-[#FDFBF7] shadow-[0_14px_40px_rgba(131,140,127,0.2)]">
      <motion.div
        ...
        animate={
          shouldReduceMotion
            ? { scale: phaseScaleMap[currentPhase] }
            : { scale: phaseScaleMap[currentPhase], boxShadow: phaseGlowMap[currentPhase] }
        }
        transition={{ duration: shouldReduceMotion ? 0.2 : 1.8, ease: 'easeInOut' }}
      >
        {children}
      </motion.div>
    </div>
